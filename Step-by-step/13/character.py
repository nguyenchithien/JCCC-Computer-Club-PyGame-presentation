...
class NPC( Player ):
    ...
    def __init__( self, image, x, y, width, height ):
        Player.__init__( self, image, x, y, width, height )
        self.goalX = -1
        self.goalY = -1

    def __init__( self, image, x, y, width, height, speed ):
        Player.__init__( self, image, x, y, width, height, speed )
        self.goalX = -1
        self.goalY = -1

    def SetGoal( self, goalX, goalY ):
        self.goalX = goalX
        self.goalY = goalY
    ...
    def Update( self ):
        ...
        if ( self.goalX == -1 or self.goalY == -1 ):
            return

        if ( self.goalX < self.x - 5 ):
            self.Move( -self.speed, 0 )
            self.direction = 2

        elif ( self.goalX > self.x + 5 ):
            self.Move( self.speed, 0 )
            self.direction = 3

        if ( self.goalY < self.y - 5 ):
            self.Move( 0, -self.speed )
            self.direction = 1

        elif ( self.goalY > self.y + 5 ):
            self.Move( 0, self.speed )
            self.direction = 0
